    ...
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <div className={`relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col transition-transform duration-300 ease-in-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col items-center text-center pt-12">
          <div className="w-24 h-24 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-3xl font-extrabold mb-4 border-4 border-white shadow-lg">
            {patient.name.charAt(0)}
          </div>
          <h2 className="text-2xl font-extrabold text-slate-800">{patient.name}</h2>
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mt-1">{patient.id}</p>
          
          <div className="flex items-center gap-3 mt-4">
            <span className="px-3 py-1 bg-white border border-slate-200 text-slate-600 rounded-full text-xs font-bold shadow-sm">{patient.age} Years</span>
            <span className="px-3 py-1 bg-white border border-slate-200 text-slate-600 rounded-full text-xs font-bold shadow-sm">{patient.gender}</span>
